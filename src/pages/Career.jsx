...
const Career = () => {
  ...
  // Lock scrolling when the modal popup is active
  useEffect(() => {
    if (isModalOpen) {
  ...
  return (
    ...
    <Helmet>
              ...
              {openRoles.map((role, i) => (
                ...
                >
                  <div className="mb-6 md:mb-0">
                    <h3 className="text-2xl font-bold group-hover:text-[#de8f32] transition-colors mb-2">
                      {role.title}
                    </h3>
                    <div className="flex gap-4 text-xs font-bold text-gray-500 uppercase tracking-widest">
                      ...
                      <span className="flex items-center gap-1 font-black text-[#015aae] italic">
                        {role.location}
                      </span>
                    </div>
                  </div>
                  <div className="flex items-center gap-6">
                    <div className="hidden lg:flex gap-2">
                      {role.stack.map((tech, idx) => (
                        <span
                          key={idx}
                          className="px-3 py-1 rounded-md bg-black/20 border border-white/5 text-[10px] text-gray-400"
                        >
                          {tech}
                        </span>
                      ))}
          ...
          >
            <div className="relative z-10 max-w-2xl">
              ...
              <p className="text-gray-400 text-lg mb-10">
                We are always looking for "Core" talent. Send your resume and a
                brief intro about your favorite tech stack to our talent team.
              </p>
              <button 
                onClick={() => setIsModalOpen(true)}
                className="px-10 py-4 bg-[#de8f32] text-[#031c36] font-black rounded-2xl hover:scale-105 transition-all shadow-xl shadow-[#de8f32]/20"
              >
      ...
      {/* Frame Motion wrapper prevents abrupt unmounting loops */}
      <AnimatePresence>
        {isModalOpen && (
          <CareerForm isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
        )}
      </AnimatePresence>
    </>
  );
